...
function init(gl, buf) {
    ...
    // Debug UI (only if debug div exists)
    let debugDiv = document.createElement("div")
    debugDiv.classList.add('debug')
    debugInfo.debugDiv = debugDiv
    let debugElement = document.querySelector("#debug_div");
    if (debugElement) {
        debugElement.appendChild(debugDiv);
    }
}
...
function update() {
    ...
    if (player.isDead) {
        ...
        player.speed.mixEq(speedMul * player.reqSpeed.x, speedMul * player.reqSpeed.y, 0.25);
...
function render(gl) {
    let dt = [1 / 70, 1 / 60, 1 / 50, 1 / 40][gameSettings.difficulty]
    ctx.time += dt
    if (gameState == STATE_MENU || gameState == STATE_START_CUTSCENE || gameState == STATE_END) {
        ...
        // solid check
        if (pixelValues[0] > 1 && !debugInfo.noclip) {
        ...
        // death check
        if (pixelValues[4] > 1 && !isCheckpoint && !player.isDead && !debugInfo.godmode) {
            player.isDead = true
